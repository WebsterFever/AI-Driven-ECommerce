...
export interface ProductFormValues { name: string; description: string; price: number; category: Category; imageUrl: string; stock: number }
...
function ProductForm({ initialValues, submitLabel, onSubmit }: ProductFormProps) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setLoading(true)
    try { await onSubmit({ name, description, price: Number(price), category, imageUrl, stock: Number(stock) }) }
  ...
  return <form onSubmit={handleSubmit} className="flex max-w-lg flex-col gap-4">
    ...
    <label className="flex flex-col gap-1 text-sm font-medium text-slate-700">Stock<input type="number" step="1" min="0" value={stock} onChange={(e) => setStock(e.target.value)} required className="rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" /></label>
