...
function OrderConfirmation() {
  ...
  return <div className="flex min-h-screen flex-col items-center justify-center gap-3 bg-slate-50 px-4 text-center">
    <h1 className="text-2xl font-bold text-slate-900">Order placed successfully!</h1>
    <p className="text-sm text-slate-500">Order number: {order.id}</p>
    <p className="text-sm text-slate-500">Status: {order.status}</p>
    <p className="text-lg font-bold text-blue-600">{formattedTotal}</p>
    <Link to="/" className="mt-4 font-medium text-blue-600 hover:underline">Back to catalog</Link>
  </div>
}
